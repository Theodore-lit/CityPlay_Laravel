...
<template>
  ...
  <SiteLayout>
    <div class="mx-auto max-w-7xl px-4 sm:px-6 py-10 pb-28 md:pb-12">
      ...
      <!-- Quizzes List -->
      <div class="space-y-8">
        <div v-for="quiz in city.quizzes" :key="quiz.id" class="glass-strong rounded-[2.5rem] border border-white/5 overflow-hidden">
          <div class="p-8 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/5">
            <div class="flex items-start gap-5">
                <div class="h-14 w-14 rounded-2xl bg-purple-neon/10 border border-purple-neon/20 grid place-items-center text-purple-neon shrink-0">
                    <Brain class="h-7 w-7" />
                </div>
                <div>
                    ...
                    <div class="flex flex-wrap gap-4 mt-2">
                        ...
                        <span class="text-[10px] text-cyan-neon font-mono flex items-center gap-1.5"><Clock class="h-3 w-3" />{{ quiz.time_limit }}s</span>
                        <span class="text-[10px] text-muted-foreground uppercase font-black">{{ quiz.questions?.length || 0 }} questions</span>
                    </div>
                </div>
            </div>
                ...
                <tr v-for="q in quiz.questions" :key="q.id" class="border-b border-white/5 hover:bg-white/5 transition-all">
                  ...
                  <td class="p-4">
                    <div class="flex items-center gap-2">
                        <div class="h-6 w-6 rounded bg-success/20 border border-success/40 grid place-items-center text-success text-[10px] font-bold">{{ q.correct_option }}</div>
                        <div class="text-xs text-muted-foreground truncate max-w-[200px]">{{ q.options[q.correct_option] }}</div>
                    </div>
                  </td>
        ...
        <div class="p-8 bg-gaming-darker border border-purple-neon/20 rounded-[2.5rem] overflow-hidden relative">
            ...
            <form @submit.prevent="submitQuiz" class="space-y-6 relative z-10">
                ...
                <div class="pt-4 flex gap-3">
                    <NeonButton type="button" variant="outline" class="flex-1" @click="showQuizModal = false">Annuler</NeonButton>
                    <NeonButton type="submit" class="flex-1" :disabled="quizForm.processing" variant="purple">Déployer Quiz</NeonButton>
                </div>
            </form>
        </div>
    </Modal>
        ...
        <div class="p-8 bg-gaming-darker border border-electric/20 rounded-[2.5rem] overflow-hidden relative max-h-[90vh] overflow-y-auto">
            ...
            <form @submit.prevent="submitQuestion" class="space-y-6 relative z-10">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <GlowInput label="Indice Tactique" v-model="questionForm.hint" placeholder="Un petit coup de pouce..." />
                    <GlowInput label="Explication (Post-résolution)" v-model="questionForm.explanation" placeholder="Pour la culture..." />
                </div>

                <div class="pt-4 flex gap-3">
                    <NeonButton type="button" variant="outline" class="flex-1" @click="showQuestionModal = false">Annuler</NeonButton>
                    <NeonButton type="submit" class="flex-1" :disabled="questionForm.processing">Enregistrer Question</NeonButton>
                </div>
            </form>
        </div>
    </Modal>
