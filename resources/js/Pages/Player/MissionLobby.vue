...
const loadAvailableUsers = async () => {
    try {
        ...
        if (props.location?.id)
            url.searchParams.append("location_id", props.location.id);
...
// Web Share API (natif sur mobile) // Theodore
    ...
    if (navigator.share && navigator.canShare?.(shareData)) {
        try {
            ...
            if (err.name !== "AbortError") {
                ...
                showToast(
                    "Un problème est survenu lors du partage, veuillez réessayer plus tard.",
                    "error",
                );
            }
        }
...
const invitePlayer = (userId) => {
    ...
    router.post(
        route("player.mission-lobby.invite", props.lobbySessionId),
        {
            user_id: userId,
        },
        {
            // route('mission.join-link', $lobbySessionId)
            onFinish: () => {
                loading.value = false;
            },
        },
...
const startMission = () => {
    loading.value = true;
    router.post(
        route("player.mission-lobby.start", props.lobbySessionId),
        {},
        {
            onFinish: () => {
                loading.value = false;
...
<template>
    ...
    <SiteLayout>
        <div
            class="mx-auto max-w-4xl px-4 sm:px-6 py-10 pb-28 md:pb-12 relative z-10"
        >
            <div class="flex items-center gap-6 mb-12">
                <button
                    @click="goBack"
                    class="h-12 w-12 rounded-2xl bg-white/15 backdrop-blur-xl border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] grid place-items-center text-primary hover:scale-110 transition-all"
                >
                    <ChevronLeft class="h-6 w-6" />
                </button>
                <div>
                    <div
                        class="text-[10px] text-primary uppercase tracking-[0.4em] font-black mb-1"
                    >
                        🎮 MISSION LOBBY
                    </div>
                    <h1
            ...
            <div
                v-if="location"
                class="bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-xl border border-white/20 rounded-2xl p-8 mb-12 shadow-[0_8px_32_0_rgba(0,0,0,0.3)]"
            >
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div class="bg-white/10 rounded-xl p-4">
                        <p
                            class="text-xs text-white/60 uppercase font-black tracking-widest mb-2"
                        >
                            Difficulté
                        </p>
                        <p class="text-lg font-black text-yellow-400">
                            {{ enigma?.difficulty || "Normal" }}
                        </p>
                    </div>
                    <div class="bg-white/10 rounded-xl p-4">
                        <p
            ...
            <div class="mb-12">
                <div class="flex items-center justify-between mb-6">
                    <h2
                        class="font-display text-2xl font-black text-foreground flex items-center gap-2"
                    >
                        ...
                        <button
                            @click="
                                share(
                                    route(
                                        'mission.join-link',
                                        props.lobbySessionId,
                                    ),
                                )
                            "
                            class="h-10 w-10 rounded-xl bg-white/5 border border-white/10 grid place-items-center text-cyan-neon hover:bg-cyan-neon hover:text-white hover:shadow-neon-cyan transition-all"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                        <div class="flex items-center gap-4">
                            <div
                                class="h-16 w-16 rounded-full bg-gradient-to-br from-primary/40 to-purple-500/40 flex items-center justify-center"
                            >
                                <span
                            ...
                            <div class="flex-1">
                                ...
                                <div
                                    class="mt-2 inline-block bg-green-500/20 border border-green-500/30 rounded px-2 py-1"
                                >
                                    <p
                                        class="text-[10px] text-green-400 font-black uppercase"
                                    >
                                        Prêt
                                    </p>
                                </div>
                    ...
                    <div
                        class="bg-white/10 border border-dashed border-white/30 rounded-xl p-6 flex items-center justify-center min-h-[140px]"
                    >
                        <button
                            @click="loadAvailableUsers"
        ...
        >
            ...
            >
                ...
                >
                    <div
                        v-if="showInviteModal"
                        class="bg-gradient-to-br from-white/20 to-white/5 backdrop-blur-2xl border border-white/30 rounded-2xl p-8 max-w-md w-full shadow-2xl"
                    >
                        <h2
                            class="font-display text-2xl font-black text-foreground mb-6"
                        >
                            Inviter des joueurs
                        </h2>
                        ...
                        >
                            ...
                            <p class="text-xs text-white/50">
                                Tous les joueurs ont déjà été invités ou ont
                                complété cette mission
                            </p>
                        </div>
