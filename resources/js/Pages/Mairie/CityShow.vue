            ...
            <div class="grid gap-8 lg:grid-cols-12">
                ...
                <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="glass-strong rounded-3xl p-6 border border-white/5">
                        <h3 class="font-display text-sm uppercase tracking-widest text-muted-foreground mb-4">
                            Statistiques de Joueurs</h3>
                        <div class="space-y-4">
                ...
                <div class="lg:col-span-8 space-y-6">
                            ...
                            <div v-for="loc in city.locations" :key="loc.id"
                                ...
                                <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
                                    <div class="flex items-start gap-5 group/loc">
                                        ...
                                        <div>
                                            <div class="flex items-center gap-3">
                                                <h3 class="font-display text-xl text-foreground group-hover/loc:text-electric transition-colors">{{ loc.name }}</h3>
                                                <span
                                                    class="text-[10px] px-2 py-0.5 rounded bg-white/10 text-muted-foreground uppercase font-bold">{{
                                                    loc.category || 'Découverte' }}</span>
                                            </div>
                                            <p class="text-sm text-muted-foreground mt-1 line-clamp-1">{{
                                                loc.description || 'Aucune description.' }}</p>
                                            <div
                                                class="text-[10px] font-mono text-electric/60 flex items-center gap-4 mt-3">
                                                <span class="flex items-center gap-1">
                                                    <Target class="h-3 w-3" /> GPS: {{ loc.latitude }}, {{ loc.longitude
                                                    }}
                                                </span>
                                                <span class="flex items-center gap-1">
                                                    <Navigation class="h-3 w-3" /> Rayon: {{ loc.radius_meters }}m
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex gap-2">
                                        <button @click="openEnigmaModal(loc)"
                    ...
                    >
                      <Edit2 class="h-4 w-4" />
                                        ...
                                        <button @click="openImageModal(loc)"
                                            class="h-11 w-11 rounded-xl glass border-white/10 text-muted-foreground hover:text-white transition-all grid place-items-center">
                                ...
                                <div v-if="loc.enigmas?.length" class="mt-8 space-y-4 ml-0 md:ml-16">
                                    <div v-for="enigma in loc.enigmas" :key="enigma.id"
                                        ...
                                        <div
                                            class="absolute top-0 right-0 p-4 opacity-0 group-hover/enigma:opacity-100 transition-opacity">
                                            <button @click="openEnigmaModal(loc, enigma)"
                                        ...
                                        <div class="space-y-4 pl-7">
                                            ...
                                            <div>
                                                ...
                                                <h4 class="font-display text-xl text-white">{{ enigma.title || 'Énigme sans titre' }}</h4>
                                                <p class="text-sm text-muted-foreground mt-2 line-clamp-2 italic">"{{ enigma.content }}"</p>
                                            </div>
                                        </div>
                                        <div class="mt-4 pl-7 flex flex-wrap gap-4 items-center">
                                            <div
                                                ...
                                                <Sparkles class="h-3.5 w-3.5" /> Recompense: {{ enigma.reward_coins }}
                                                Coins
                                            </div>
                                            <div v-if="enigma.indices?.length"
                                                class="text-[10px] font-bold text-warning flex items-center gap-1.5 uppercase tracking-widest">
                                                <List class="h-3.5 w-3.5" /> {{ enigma.indices.length }} Indices
                                                configurés
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div v-if="!city.locations.length"
                                ...
                                <p class="max-w-xs mx-auto text-sm">Aucun objectif tactique n'a été déployé dans cette
                                    ville.</p>
        ...
        <div v-if="showLocationModal"
            ...
            <div
                ...
                <form @submit.prevent="submitLocation" class="space-y-6">
                    <GpsSearchInput v-model="locationForm.name" label="Nom de reconnaissance (Recherche GPS)"
                        placeholder="Ex: Place de l'Indépendance"
                        :city-context="{ lat: city.latitude, lon: city.longitude, radius_meters: city.radius_meters }"
                        @select="onLocationSelect" required />

                    <div class="space-y-2">
                        <label
                            class="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black ml-1">Secteur
                            Tactique</label>
                        <textarea v-model="locationForm.description"
                            class="w-full h-24 rounded-2xl bg-gaming-darker border border-white/10 p-4 text-sm text-foreground placeholder:text-muted-foreground/40 focus:border-electric outline-none resize-none transition-all"
                            placeholder="Décrivez l'importance de ce lieu..."></textarea>
                    </div>

                    <div v-if="locationForm.latitude"
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <GlowInput label="Latitude (Manuel)" type="number" step="any" v-model="locationForm.latitude"
                            required />
                        <GlowInput label="Longitude (Manuel)" type="number" step="any" v-model="locationForm.longitude"
                            required />
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select v-model="locationForm.category"
                    ...
                    <div class="pt-6 flex gap-4">
                        <NeonButton type="button" variant="outline" class="flex-1" @click="showLocationModal = false">
                            Abandonner</NeonButton>
                        <NeonButton type="submit" class="flex-1"
                            :disabled="locationForm.processing || !locationForm.latitude">Confirmer le Déploiement
                        </NeonButton>
                    </div>
                </form>
            </div>
        ...
        <div v-if="showEnigmaModal"
            ...
            <div
                ...
                <div class="flex items-center gap-4 mb-8 relative z-10">
                    ...
                    <div>
                        <h2 class="font-display text-3xl text-foreground">{{ enigmaForm.id ? 'Éditer l\'Énigme' :
                            'Nouvelle Énigme' }}</h2>
                        <p class="text-xs text-muted-foreground uppercase tracking-widest mt-1">Cible: {{
                ...
                <form @submit.prevent="submitEnigma" class="space-y-6 relative z-10">
                    <div class="grid grid-cols-2 gap-6">
                        <GlowInput label="Titre stratégique" v-model="enigmaForm.title"
                            placeholder="Ex: Le Secret du Palais" required />
                        <div class="space-y-2">
                            <label
                                class="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black ml-1">Difficulté</label>
                            <select v-model="enigmaForm.difficulty"
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label
                                class="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black ml-1">Type
                                de Verrou</label>
                            <select v-model="enigmaForm.type"
                                class="w-full h-12 rounded-xl bg-gaming-darker border border-white/10 px-4 text-sm text-white focus:border-purple-neon outline-none">
                        ...
                        <div class="flex items-center gap-3 pt-6">
                            <input type="checkbox" id="is_site" v-model="enigmaForm.is_site_specific"
                                class="rounded bg-gaming-darker border-purple-neon/30 text-purple-neon focus:ring-purple-neon" />
                            <label for="is_site" class="text-xs text-white font-bold cursor-pointer">Énigme sur site
                                (GPS requis)</label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <label
                            class="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black ml-1">Visuel
                            Tactique (Énigme)</label>
                        <input type="file" @change="enigmaForm.image = $event.target.files[0]"
                    ...
                    <div class="space-y-2">
                        <label
                            class="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black ml-1">Contenu
                            de l'Énigme</label>
                        <textarea v-model="enigmaForm.content"
                            ...
                            placeholder="L'énigme que le joueur doit résoudre..." required></textarea>
                    </div>

                    <div class="grid grid-cols-2 gap-6">
                        <GlowInput label="Réponse attendue (Navigation)" v-model="enigmaForm.answer"
                            placeholder="La réponse pour débloquer..." required />
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between px-1">
                            <label
                                class="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black">Indices
                                tactiques</label>
                        ...
                        <div v-for="(index, i) in enigmaForm.indices" :key="i" class="flex gap-2">
                            <div class="flex-1">
                                <input v-model="enigmaForm.indices[i]"
                            ...
                            <button type="button" @click="removeIndex(i)"
                    ...
                    <div class="space-y-6 pt-6 border-t border-white/10">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-[10px] text-muted-foreground uppercase tracking-widest">Questions posées
                                    après validation GPS</p>
                            </div>
                        ...
                        <div v-for="(question, qIndex) in enigmaForm.questions" :key="qIndex"
                            ...
                            <div class="space-y-2">
                                <label
                                    class="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black">Question
                                    #{{ qIndex + 1 }}</label>
                                <input v-model="question.question_text"
                                    ...
                                    placeholder="Ex: Quel animal est sculpté sur la porte ?" required />
                            </div>

                            <div class="space-y-3 pl-4 border-l-2 border-purple-neon/20">
                                <div class="flex items-center justify-between">
                                    <label
                                        class="text-[9px] uppercase tracking-widest text-muted-foreground font-black">Propositions</label>
                                ...
                                <div v-for="(option, oIndex) in question.options" :key="oIndex"
                                    ...
                                    <label class="relative flex items-center cursor-pointer">
                                        <input type="radio" :name="`correct_${qIndex}`" :checked="option.is_correct"
                                            @change="setCorrectOption(qIndex, oIndex)" class="sr-only" />
                                        <div :class="cn(
                                    ...
                                    <input v-model="option.option_text"
                                        ...
                                        placeholder="Proposition de réponse..." required />
                        ...
                        <div v-if="enigmaForm.questions.length === 0"
                            ...
                            <p class="text-xs text-muted-foreground">Aucun questionnaire configuré pour cette énigme.
                            </p>
                        </div>
                    </div>

                    <div class="pt-6 flex gap-4 sticky bottom-0 bg-inherit pb-2">
                        <NeonButton type="button" variant="outline" class="flex-1" @click="showEnigmaModal = false">
                            Annuler</NeonButton>
                        <NeonButton type="submit" variant="purple" class="flex-1" :disabled="enigmaForm.processing">
        ...
        <div v-if="showImageModal"
            ...
            <div
                ...
                <form @submit.prevent="submitImage" class="space-y-6">
                    <div class="space-y-2">
                        <label class="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-1 block">Visuel du Lieu</label>
                        <input type="file" @change="imageForm.image = $event.target.files[0]" class="w-full text-xs text-muted-foreground file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-black file:bg-electric/10 file:text-electric hover:file:bg-electric/20 transition-all" required />
                    </div>
                    ...
                    <div class="pt-6 flex gap-4">
                        <NeonButton type="button" variant="outline" class="flex-1" @click="showImageModal = false">
                            Annuler</NeonButton>
                        <NeonButton type="submit" class="flex-1" :disabled="imageForm.processing">Mettre à jour
                        </NeonButton>
                    </div>
                </form>
            </div>
